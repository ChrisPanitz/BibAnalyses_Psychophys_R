...
pubPlot <- ggplot(data = dfPubs, aes(x = year, y = nrPubs)) +
                  ...
                  geom_text(aes(y = nrPubs + yMaxPub/100, label = nrPubs), size = 2, fontface = "bold", color = "black") +
                  scale_x_continuous(name = "Year", limits = c(1963,2024), breaks = seq(2*ceiling(min(dfPubs$year)/2),max(dfPubs$year),2)) +
...
#dfSub$nrSubmissions <- round(dfIF$IF, 2)
dfSub
...
subPlot <- ggplot(data = dfSub, aes(x = year, y = submissions)) +
  theme_classic() +
  geom_col(fill = "gray70") + 
  #geom_text(aes(y = 5, label = submissions), size = 2, fontface = "bold", color = "white") +
  geom_text(aes(y = submissions + yMaxSub/100, label = submissions), size = 2, fontface = "bold", color = "black") +
  #scale_x_continuous(name = "Year", breaks = seq(min(dfSub$year),max(dfSub$year),2)) +
  scale_x_continuous(name = "Year", limits = c(1963, 2024), breaks = seq(2*ceiling(min(dfSub$year)/2),max(dfSub$year),2)) +
...
ifPlot <- ggplot(data = dfIF, aes(x = year, y = IF)) +
                 theme_classic() +
                 geom_col(fill = "gray70") +
                 geom_text(aes(y = IF + yMaxIF/100, label = IF), size = 2, fontface = "bold", color = "black") +
                 scale_x_continuous(name = "Year", limits = c(1963, 2024), breaks = seq(2*ceiling(min(dfIF$year)/2),max(dfIF$year),2)) +
...
test <- rep(NA,58)
for (i in 1966:2023){
  test[i-1965] <- sum(df$TC[(df$PY == i-2) | (df$PY == i-1)]) / 
                    (sum(df$PY == i-2) + sum(df$PY == i-1))
}
